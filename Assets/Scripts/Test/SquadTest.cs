    ...
    public class SquadTest : MonoBehaviour
    {
        ...
        void TestSquadConfiguration()
        {
            ...
            for (int i = 0; i < 20; i++) // Try to add more than max
            {
                ...
                testUnit.characterId = i.ToString();
                testUnit.unitName = $"Unit {i}";
        ...
        void TestBattleManager()
        {
            ...
            Debug.Log($"Squads per Side: {GuildMaster.Core.BattleManager.SQUADS_PER_SIDE} (Expected: 2)");
            Debug.Log($"Squad Rows: {GuildMaster.Core.BattleManager.SQUAD_ROWS} (Expected: 3)");
            Debug.Log($"Squad Cols: {GuildMaster.Core.BattleManager.SQUAD_COLS} (Expected: 6)");
            Debug.Log($"Units per Squad: {GuildMaster.Core.BattleManager.UNITS_PER_SQUAD} (Expected: 18)");
            Debug.Log($"Total Units per Side: {GuildMaster.Core.BattleManager.TOTAL_UNITS_PER_SIDE} (Expected: 36)");
        }
