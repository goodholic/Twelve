...
namespace GuildMaster.Battle
{
    ...
    public class AIBehaviorSO : ScriptableObject
    {
        ...
        public float EvaluateTarget(UnitState unit, UnitState target, BattleContext context)
        {
            float score = 0f;
        ...
        float EvaluateTargetCondition(UnitState unit, UnitState target, TargetPriority.TargetCondition condition, BattleContext context)
        {
            switch (condition)
            {
        ...
        public bool ShouldUseSkill(string skillId, UnitState unit, BattleContext context)
        {
        ...
        bool EvaluateSkillCondition(SkillUsageRule.SkillCondition condition, float threshold, UnitState unit, BattleContext context)
        {
            switch (condition)
            {
        ...
        public Vector3 GetMovementPosition(UnitState unit, BattleContext context)
        {
            switch (movementStrategy)
            {
                ...
                case MovementStrategy.ProtectAllies:
                    UnitState weakestAlly = GetWeakestAlly(context.allies);
                    if (weakestAlly != null)
        ...
        UnitState GetWeakestAlly(List<UnitState> allies)
        {
            UnitState weakest = null;
        ...
        Vector3 GetStrategicPosition(UnitState unit, BattleContext context)
        {
        ...
        Vector3 GetTeamCenter(List<UnitState> units)
        {
    ...
    {
        public List<UnitState> allies = new List<UnitState>();
        public List<UnitState> enemies = new List<UnitState>();
        public UnitState currentTarget;
