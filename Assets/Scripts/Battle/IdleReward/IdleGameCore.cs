...
namespace GuildMaster.Systems
{
    ...
    public class IdleGameCore : MonoBehaviour
    {
        ...
        private IEnumerator AutoResourceManagement(AutomationReport report)
        {
            ...
            // GetResources 및 HasBuilding 메서드가 제거되어 주석 처리
            // var resources = guild.GetResources();
            // 
            // // 자원 균형 조정
            // if (resources.gold < 1000 && guild.HasBuilding("Storage"))
            // {
            ...
            report.actions.Add("자원 관리 시스템 비활성화");
        ...
        private IEnumerator AutoGuildManagement(AutomationReport report)
        {
            ...
            if (guild.GetGuildLevel() < 50)
            {
                report.actions.Add($"길드 레벨: {guild.GetGuildLevel()}");
            }
        ...
        private IEnumerator AutoBuildingManagement(AutomationReport report)
        {
            ...
            var guild = GuildSimulationCore.Instance;
            // GetResources, HasBuilding, ConstructBuilding, UpgradeBuilding, GetBuildings 메서드가 제거되어 주석 처리
            // var resources = guild.GetResources();
            // 
            // // 필수 건물 우선 건설
            // var priorityBuildings = new List<string>
            // {
            //     "Storage",      // 자원 저장
            //     "Mine",         // 석재 생산
            //     "Lumbermill",   // 목재 생산
            ...
            report.actions.Add("건설 시스템 비활성화");
        ...
        private void CalculateOfflineRewards(float offlineHours)
        {
            ...
            // 임시로 기본값 설정
            reward.goldEarned = 1000 * offlineHours * offlineRewardMultiplier;
            reward.expEarned = 100 * offlineHours * offlineRewardMultiplier;
            reward.battlesWon = Mathf.RoundToInt(offlineHours * 2 * offlineRewardMultiplier);
            ApplyOfflineReward(reward);
        ...
        private void ApplyOfflineReward(OfflineReward reward)
        {
            ...
            if (GuildSimulationCore.Instance != null)
            {
                ...
                PlayerPrefs.SetInt("Gold", currentGold + Mathf.RoundToInt(reward.goldEarned));
        ...
        // TODO: Replace GuildResources with proper implementation
        /*
        private bool CanAffordBuilding(string buildingType, object resources)
        {
            // TODO: Implement proper resource checking when GuildResources is available
            // 건물 건설 비용 체크
            return true; // 임시로 true 반환
        }
        */
        
        private Vector2Int? FindBestBuildingPosition(GuildSimulationCore guild, string buildingType)
        {
            // 최적 건설 위치 찾기 로직
