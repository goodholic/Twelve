...
namespace GuildMaster.Systems
{
    ...
    public class IdleRewardSystem : MonoBehaviour
    {
        ...
        public class IdleRewardData
        {
            ...
            // public Dictionary<ResourceType, int> resources; // ResourceType removed
            public Dictionary<string, int> resources; // Using string instead of ResourceType
            ...
            {
                resources = new Dictionary<string, int>(); // Changed from ResourceType to string
        ...
        IdleRewardData CalculateIdleRewards(float seconds)
        {
            ...
            */
            ...
            // 임시 기본 자원 생산
            rewards.resources["Gold"] = Mathf.FloorToInt(goldPerMinute * (seconds / 60f) * efficiency); // Changed from ResourceType.Gold to string
        ...
        float CalculateIdleEfficiency()
        {
            ...
            if (guildManager != null)
            {
                ...
                // }
                // Using string instead of BuildingType enum
                efficiency *= 1f + (guildManager.GetBuildingLevel("Shop") * 0.1f);
            }
        ...
        public IdleRewardData ClaimIdleRewards()
        {
            ...
            var claimedRewards = new IdleRewardData
            {
                ...
                resources = new Dictionary<string, int>(accumulatedRewards.resources), // Changed from ResourceType to string
            ...
            // {
            //     resourceManager?.AddResource(resource.Key, resource.Value, "Idle Reward"); // ResourceManager commented out
            // }
        ...
        IdleRewardData GenerateActivityRewards(ActivityType type, int maxProgress)
        {
            ...
            switch (type)
            {
                case ActivityType.AutoExploration:
                    rewards.resources["Gold"] = maxProgress * 10; // Changed from ResourceType.Gold
                    rewards.totalExperience = maxProgress * 5;
                    break;
                    
                case ActivityType.ResourceGathering:
                    rewards.resources["Wood"] = maxProgress * 5; // Changed from ResourceType.Wood
                    rewards.resources["Stone"] = maxProgress * 3; // Changed from ResourceType.Stone
                    break;
                ...
                case ActivityType.Research:
                    rewards.resources["ManaStone"] = maxProgress / 10; // Changed from ResourceType.ManaStone
                    break;
            }
        ...
        void LoadIdleData()
        {
            if (PlayerPrefs.HasKey("IdleSystem_LastActiveTime"))
            {
                ...
                // 자원 로드
                // foreach (ResourceType type in Enum.GetValues(typeof(ResourceType))) // ResourceType removed
                // {
                ...
                //     }
                // }
                // Temporary resource loading for known resources
                string[] knownResources = { "Gold", "Wood", "Stone", "ManaStone" };
                foreach (string resourceName in knownResources)
                {
                    string key = $"IdleSystem_Resource_{resourceName}";
                    if (PlayerPrefs.HasKey(key))
                    {
                        accumulatedRewards.resources[resourceName] = PlayerPrefs.GetInt(key);
                    }
                }
