...
public class WaveSpawner : MonoBehaviour
{
    ...
    /// <summary>
    /// 몬스터 생성
    /// ★★★ 핵심 로직: 웨이브별 몬스터 생성 및 경로 할당
    /// - 3마리 중 마지막 몬스터는 다음 챕터의 몬스터로 생성 (프리뷰 기능)
    /// - 3가지 경로(좌/중/우) 중 랜덤 배정
    /// - 챕터별로 다른 몬스터 프리팹 사용
    /// </summary>
    private void SpawnMonster(int indexInWave, int routeIndex)
    {
        if (monsterPrefab == null || monsterParent == null)
        {
            ...
            aliveMonsters--;  // 생성 실패 시 카운트 감소하여 웨이브 진행이 막히지 않도록 함
            return;
        }

        // ★ 게임 기획서 요구사항: 3라인 시스템 - 선택된 라우트의 웨이포인트 획득
        ...
        // ★ 챕터 시스템: 매 웨이브 3마리 중 마지막 몬스터는 다음 챕터의 몬스터로 소환
        // 이를 통해 플레이어는 다음 챕터의 몬스터를 미리 경험할 수 있음
        ...
        newMonster.name = $"Monster_W{currentWave}_{indexInWave}"; // 디버깅용 이름 설정

        Monster monster = newMonster.GetComponent<Monster>();
        if (monster != null)
        {
            // 메모리 관리를 위해 생성된 몬스터 추적
            spawnedMonsters.Add(monster);
            
            // 몬스터 기본 설정
            monster.areaIndex = 1;  // 지역1 몬스터 (플레이어 진영으로 향함)
            monster.SetChapter(monsterChapter);
            
            // ★ 라우트 타입 설정: 인덱스를 RouteType enum으로 변환
            RouteType routeType = RouteType.Center;
            switch (routeIndex)
            {
                case 0: routeType = RouteType.Left; break;   // 왼쪽 경로
                case 1: routeType = RouteType.Center; break; // 중앙 경로
                case 2: routeType = RouteType.Right; break;  // 오른쪽 경로
            }
            
            // 경로 설정 - 몬스터가 따라갈 웨이포인트 배열 할당
            monster.SetPath(selectedWaypoints, routeType);

            // ★ 중요: 이벤트 구독으로 몬스터 상태 추적
            // 몬스터 사망 시 카운트 감소 및 보상 지급
        ...
        else
        {
            ...
            aliveMonsters--;  // 컴포넌트 누락 시에도 카운트 감소하여 진행 보장
        }
    }
    ...
    {
        // null이거나 죽은 몬스터 제거
        spawnedMonsters.RemoveAll(m => m == null || !m.IsAlive());
    }
    
    /// <summary>
    /// 모든 몬스터 정리
    /// </summary>
    private void CleanupAllMonsters()
    {
