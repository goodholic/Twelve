...
public class CharacterInventoryManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        Debug.LogError($"[CharacterInventoryManager] 데이터베이스 캐릭터 수: {characterDatabaseObject.characters.Length}");
        ...
        // 가챠 풀 구성
        gachaPool.Clear();
        if (characterDatabaseObject.characters != null && characterDatabaseObject.characters.Length > 0)
        {
        ...
        else
        {
            ...
            Debug.LogError($"[CharacterInventoryManager] 배열 길이: {(characterDatabaseObject.characters != null ? characterDatabaseObject.characters.Length : -1)}");
        }
        ...
        if (gachaPool.Count == 0)
        {
            ...
            if (resourceDB != null && resourceDB.characters != null && resourceDB.characters.Length > 0)
            {
                Debug.LogError($"[CharacterInventoryManager] Resources에서 발견됨: {resourceDB.characters.Length}개 캐릭터");
            ...
            if (gachaPool.Count == 0)
            {
                ...
                if (directDB != null && directDB.Length > 0)
                {
                    ...
                    {
                        if (db != null && db.characters != null && db.characters.Length > 0)
                        {
    ...
    private void CreateDefaultCharacters()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            CharacterData defaultChar = new CharacterData
            {
                ...
                maxHP = 100,
                moveSpeed = 1f,
                rangeType = (RangeType)(i % 3), // 0=Melee, 1=Ranged, 2=Magic 순환
                isAreaAttack = (i % 4 == 0), // 4번째마다 범위 공격
                isBuffSupport = (i % 5 == 0), // 5번째마다 버프 지원
                level = 1,
                currentExp = 0,
                expToNextLevel = 100,
                initialStar = (CharacterStar)1,
