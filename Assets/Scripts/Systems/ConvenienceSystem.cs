    ...
    public class ConvenienceSystem : MonoBehaviour
    {
        ...
        void ApplyReward(PendingReward reward)
        {
            ...
            foreach (var item in reward.rewards)
            {
                ...
                {
                    case "gold":
                        gameManager.ResourceManager?.AddGold(item.Value);
                        break;
                    case "wood":
                        gameManager.ResourceManager?.AddWood(item.Value);
                        break;
                    case "stone":
                        gameManager.ResourceManager?.AddStone(item.Value);
                        break;
                    case "manastone":
                        gameManager.ResourceManager?.AddManaStone(item.Value);
                        break;
                    case "reputation":
                        gameManager.ResourceManager?.AddReputation(item.Value);
                        break;
                    case "exp":
                        // TODO: 경험치 처리
                        break;
