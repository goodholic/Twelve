...
namespace GuildMaster.Systems
{
    public class AnalyticsSystem : MonoBehaviour
    {
        ...
        {
            // Earning Patterns
            public Dictionary<ResourceType, float> totalEarned = new Dictionary<ResourceType, float>();
            public Dictionary<ResourceType, float> averageEarningRate = new Dictionary<ResourceType, float>();
            public Dictionary<ResourceType, Dictionary<string, float>> earningBySources = new Dictionary<ResourceType, Dictionary<string, float>>();
            
            // Spending Patterns
            public Dictionary<ResourceType, float> totalSpent = new Dictionary<ResourceType, float>();
            public Dictionary<ResourceType, Dictionary<string, float>> spendingByCategory = new Dictionary<ResourceType, Dictionary<string, float>>();
            
            // Balance Tracking
            public Dictionary<ResourceType, List<float>> balanceHistory = new Dictionary<ResourceType, List<float>>();
            public Dictionary<ResourceType, float> peakBalance = new Dictionary<ResourceType, float>();
            public Dictionary<ResourceType, float> lowestBalance = new Dictionary<ResourceType, float>();
            
            // Economic Health
            public float inflationRate;
            public float resourceVelocity;
            public Dictionary<ResourceType, float> resourceScarcity = new Dictionary<ResourceType, float>();
        }
        ...
        public class BattleAnalytics
        {
            ...
            public Dictionary<GuildMaster.Battle.JobClass, float> classWinRates = new Dictionary<GuildMaster.Battle.JobClass, float>();
        ...
        public class CharacterAnalytics
        {
            ...
            public Dictionary<GuildMaster.Battle.JobClass, int> classCounts = new Dictionary<GuildMaster.Battle.JobClass, int>();
        ...
        {
            foreach (GuildMaster.Battle.JobClass jobClass in Enum.GetValues(typeof(GuildMaster.Battle.JobClass)))
            {
        ...
        void OnResourceCapacityReached(GameEvent gameEvent)
        {
            ...
            {
                { "resourceType", gameEvent.GetParameter<ResourceType>("resourceType") }
            });
        }
        ...
        {
            TrackAction("UnitKilled", new Dictionary<string, object>
            {
                { "eventType", "UnitKilled" }
            });
        }

        void OnUnitAttack(GuildMaster.Battle.Unit attacker, GuildMaster.Battle.Unit target, float damage)
        {
        ...
        void OnUnitHeal(GuildMaster.Battle.Unit healer, float healAmount)
        {
            battleAnalytics.totalHealingDone += (long)healAmount;
        }

        void OnUnitDeath(GuildMaster.Battle.Unit unit)
        {
            // Track unit deaths
        }
        ...
        public void TrackResourceTransaction(ResourceType type, int amount, string source, bool isEarning)
        {
            if (!enableAnalytics) return;
        ...
        public void TrackBattleParticipation(GuildMaster.Battle.Unit unit, bool isVictory)
        {
        ...
        void UpdateCharacterAnalytics(GuildMaster.Battle.Unit unit)
        {
            if (unit == null) return;
        ...
        public void CalculateEconomicMetrics()
        {
            ...
            var goldHistory = resourceAnalytics.balanceHistory[ResourceType.Gold];
        ...
        void ExportResourceAnalyticsCSV(string path, string timestamp)
        {
            ...
            foreach (ResourceType type in Enum.GetValues(typeof(ResourceType)))
            {
                var current = ResourceManager.Instance?.GetResources()?.GetResource(type) ?? 0;
        ...
        public Dictionary<string, object> GenerateAnalyticsReport()
        {
            ...
            {
                totalGoldEarned = resourceAnalytics.totalEarned[ResourceType.Gold],
                totalGoldSpent = resourceAnalytics.totalSpent[ResourceType.Gold],
                goldInflationRate = resourceAnalytics.inflationRate,
                resourceVelocity = resourceAnalytics.resourceVelocity,
                topEarningSource = GetTopResourceSource(ResourceType.Gold, true),
                topSpendingCategory = GetTopResourceSource(ResourceType.Gold, false)
            };
        ...
        string GetTopResourceSource(ResourceType type, bool isEarning)
        {
