...
public class DeckPanelManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            Debug.Log($"[DeckPanelManager] Start: 캐릭터 데이터베이스 객체 설정됨. 등록된 캐릭터 수: {(characterDBObject.characters != null ? characterDBObject.characters.Count : 0)}");
        }
        else
        {
    ...
    private void AutoFillDeckByRaceRule()
    {
        ...
        Debug.LogError($"[DeckPanelManager] ===== AUTO FILL DEBUG START =====");
        Debug.LogError($"[DeckPanelManager] CharacterInventoryManager 참조 상태: {characterInventory != null}");
        
        if (characterInventory == null)
        {
            Debug.LogError("[DeckPanelManager] CharacterInventoryManager가 null입니다!");
            Debug.LogError("[DeckPanelManager] GameObject를 찾아보겠습니다...");
            
            var foundInventory = FindFirstObjectByType<CharacterInventoryManager>();
            if (foundInventory != null)
            {
                Debug.LogError($"[DeckPanelManager] CharacterInventoryManager를 찾았습니다: {foundInventory.name}");
                characterInventory = foundInventory;
            }
            else
            {
                Debug.LogError("[DeckPanelManager] CharacterInventoryManager를 찾을 수 없습니다!");
                return;
            }
        }
        
        Debug.LogError($"[DeckPanelManager] CharacterInventoryManager 이름: {characterInventory.name}");
        Debug.LogError($"[DeckPanelManager] CharacterInventoryManager 활성 상태: {characterInventory.gameObject.activeInHierarchy}");
        
        List<CharacterData> availableChars = characterInventory.GetOwnedCharacters();
        Debug.Log($"[DeckPanelManager] 인벤토리에서 가져온 캐릭터 수: {(availableChars != null ? availableChars.Count : 0)}");
        
        if (availableChars == null || availableChars.Count == 0)
        {
            Debug.LogError("[DeckPanelManager] 인벤토리에 캐릭터가 없습니다. (자동등록 불가)");
            Debug.Log("[DeckPanelManager] CharacterInventoryManager 데이터베이스 연결 상태 확인...");
            if (characterInventory.characterDatabaseObject == null)
            {
                Debug.LogError("[DeckPanelManager] CharacterInventoryManager의 characterDatabaseObject가 null입니다!");
            }
            else
            {
                Debug.Log($"[DeckPanelManager] 데이터베이스는 연결됨: {characterInventory.characterDatabaseObject.name}");
                Debug.Log("[DeckPanelManager] 인벤토리 강제 초기화 시도...");
                characterInventory.ClearAllData();
                
                // 재시도
                availableChars = characterInventory.GetOwnedCharacters();
                Debug.Log($"[DeckPanelManager] 강제 초기화 후 캐릭터 수: {(availableChars != null ? availableChars.Count : 0)}");
                
                if (availableChars == null || availableChars.Count == 0)
                {
                    Debug.LogError("[DeckPanelManager] 강제 초기화 후에도 인벤토리가 비어있습니다!");
                    return;
                }
            }
        }
    ...
    public void OnClickUse4CardsAtRandom()
    {
        ...
        // Tile은 MonoBehaviour가 아니므로 다른 방법으로 타일 정보를 가져와야 합니다
        // 임시로 PlacementManager를 통해 배치 가능한 위치를 확인합니다
        var placementManager = GuildMaster.Game.PlacementManager.Instance;
        List<Vector2Int> validPositions = new List<Vector2Int>();
        
        if (placementManager != null)
        {
            // 지역1의 배치 가능한 위치들을 가져옵니다 (예시)
            for (int x = 0; x < 10; x++)
            {
                for (int y = 0; y < 10; y++)
                {
                    Vector2Int pos = new Vector2Int(x, y);
                    // 실제 배치 가능 여부는 PlacementManager에서 확인해야 합니다
        ...
        {
            Debug.LogWarning("[DeckPanelManager] 지역1에 배치할 수 있는 위치가 없습니다!");
            return;
        }

        Debug.Log($"[DeckPanelManager] OnClickUse4CardsAtRandom() -> 지역1의 배치가능 위치 {validPositions.Count}개 중 랜덤으로 4장 소환 시도.");
    }
