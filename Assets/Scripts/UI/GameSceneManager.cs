...
public class GameSceneManager : MonoBehaviour
{
    ...
    private void SpawnHero()
    {
        ...
        // Character 컴포넌트 설정
        GuildMaster.Data.Character heroComp = heroObj.GetComponent<GuildMaster.Data.Character>();
        if (heroComp != null)
        {
            // 히어로로 강제 지정 + HP바 비활성화
            ...
            switch ((RangeType)heroCharacter.rangeType)
            {
                case RangeType.Melee:    heroComp.attackRange = 1.2f; break;
                case RangeType.Ranged:   heroComp.attackRange = 2.5f; break;
                case RangeType.Magic:    heroComp.attackRange = 4.0f; break;
                default:                 heroComp.attackRange = 1.5f; break;
            }

            // 총알 패널 연결
            var pMan = GuildMaster.Game.PlacementManager.Instance;
    ...
    {
        if (GuildMaster.Game.PlacementManager.Instance != null)
        {
            int playerCount = GuildMaster.Game.PlacementManager.Instance.GetCharacterCount(false);
            int aiCount = GuildMaster.Game.PlacementManager.Instance.GetCharacterCount(true);
