...
namespace GuildMaster.Systems
{
    public class SeasonPassSystem : MonoBehaviour
    {
        ...
        public class RewardItem
        {
            ...
            public int quantity;
            public string resourceType; // 자원 타입인 경우 (changed from ResourceType to string)
        ...
        RewardItem GenerateRewardForLevel(int level, bool isPremium)
        {
            ...
            else if (level % 5 == 0)
            {
                ...
                if (isPremium)
                {
                    ...
                    reward.resourceType = "ManaStone"; // Changed from ResourceType.ManaStone
            ...
            else
            {
                ...
                if (isPremium)
                {
                    ...
                    switch (rewardType)
                    {
                        ...
                        case 2:
                            reward.type = RewardItem.RewardType.Resource;
                            string[] resourceTypes = { "Gold", "Wood", "Stone" };
                            reward.resourceType = resourceTypes[UnityEngine.Random.Range(0, resourceTypes.Length)];
        ...
        void GenerateSeasonShop()
        {
            ...
            // 자원 팩
            string[] resourceTypeNames = { "Gold", "Wood", "Stone", "ManaStone" };
            for (int i = 0; i < resourceTypeNames.Length; i++)
            {
                string resourceType = resourceTypeNames[i];
                seasonShop.Add(new SeasonShopItem
                {
                    itemId = 10 + i,
                    item = new RewardItem
        ...
        void ApplyReward(RewardItem reward)
        {
            ...
            switch (reward.type)
            {
                ...
                case RewardItem.RewardType.Resource:
                    ...
                    //     }
                    // }
                    Debug.Log($"Resource reward: {reward.resourceType} x{reward.quantity}");
                    break;
        ...
        public void UpdateChallengesByType(string progressType, int amount = 1)
        {
            ...
            switch (progressType)
            {
                ...
                // case "dungeon_floor": // Dungeon 기능 제거됨
