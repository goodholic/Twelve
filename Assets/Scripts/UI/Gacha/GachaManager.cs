...
// {
    ...
    public class GachaManager : MonoBehaviour
    {
        ...
        /// 캐릭터 보유 여부 확인
        /// </summary>
        private bool IsCharacterOwned(string characterID)
        {
            if (CharacterInventoryManager.Instance != null)
            {
                // characterID가 숫자 형태의 문자열인 경우 int로 변환
                if (int.TryParse(characterID, out int id))
                {
                    return CharacterInventoryManager.Instance.IsCharacterOwned(id);
                }
                // 문자열 형태의 ID인 경우 이름으로 확인
                return CharacterInventoryManager.Instance.GetOwnedCharacters()
                    .Any(c => c.characterID == characterID || c.characterName == characterID);
            }
            return false;
        }
        ...
        private void PlayDrawSound(List<GachaResult> results)
        {
            ...
            if (soundToPlay != null && GuildMaster.Systems.SoundManager.Instance != null)
            {
                GuildMaster.Systems.SoundManager.Instance.PlaySFX(soundToPlay);
            }
        }
        ...
        private void AddCharactersToInventory(List<GachaResult> results)
        {
            ...
            foreach (var result in results)
            {
                ...
                {
                    // 중복 캐릭터는 조각으로 변환
                    if (int.TryParse(result.character.characterID, out int id))
                    {
                        CharacterInventoryManager.Instance.AddCharacterFragments(id, 1);
                    }
                }
            }
        }
