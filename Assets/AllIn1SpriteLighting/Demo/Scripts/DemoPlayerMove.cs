...
public class DemoPlayerMove : MonoBehaviour
{
    ...
    private void LateralMovement()
    {
        ...
        if (grounded) rb.linearVelocity = new Vector2(inputX * moveSpeed, rb.linearVelocity.y);
        else rb.linearVelocity = new Vector2(inputX * moveSpeed * airSpeedMult, rb.linearVelocity.y);
    ...
    private void JumpHandle()
    {
        ...
        if (rb.linearVelocity.y < 0) //Increase downwards speed when falling
        {
            rb.linearVelocity -= Vector2.down * Physics2D.gravity.y * fallMult * Time.deltaTime;
        }
        else if (rb.linearVelocity.y > 0 && !jumpPressed) //Increase fall when going up and not pressing jump button
        {
            rb.linearVelocity -= Vector2.down * Physics2D.gravity.y * lowJumpMult * Time.deltaTime;
        }
    }
    ...
    private void Jump()
    {
        ...
        if(grounded) rb.linearVelocity = new Vector2(rb.linearVelocity.x, jumpF);
        else rb.linearVelocity = new Vector2(rb.linearVelocity.x, jumpF * jumpFAirFactor);
    }
    ...
    public void IsGrounded()
    {
        ...
        grounded = rb.linearVelocity.y > 0.1f ? false : groundHit.collider != null;
